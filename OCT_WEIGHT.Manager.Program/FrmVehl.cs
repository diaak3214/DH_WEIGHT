...
{
    public partial class FrmVehl : OCT_WEIGHT.Manager.Common.FrmBase
    {
        ...
        private void BtnSearch_Click(object sender, EventArgs e)
        {
            ...
            //    DataSet ds = DBConn.ExecuteDataSet2("SP_FrmVehl_R", dict);
        ...
        private void FrmVehl_Load(object sender, EventArgs e)
        {
        ...
        public void QueryClick()
        {
            try
            {
                ...
                //              + "     ON A.TRS_PLAN_NO = B.PK_FST_NO"
                //              + "    AND B.USE_YN = 'Y'"
                //              //받는 쪽은 신평(1100) 으로 박아 넣음
                //              + "  WHERE A.TO_PLNT_NO =  '1100'"
                //              //+ "  WHERE A.FROM_PLNT_NO =  '1200'"
                //              + "    AND A.SHIP_GB =  'S'"
                //              + "    AND A.ORD_GB =  'M'"
                //              + "    AND A.ITEM_TYPE IN ('10','30','50')"
                ...
                dict.Add("P_SITE_NM", txtSITE_NM.Text); //검색조건에 도(상)착 추가(2020-04-02 한민호)
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    //포커스가 제일 마지막 행에 오게 수정(2020-04-07 한민호)
                    gvwMaster.MoveBy(ds.Tables[0].Rows.Count); 
                }
                else
                {
        ...
        private void btnReceipt_Click(object sender, EventArgs e)
        {
            ...
            //계량대기로 넘어가기 때문에 수입고철도 동일하게 적용(2020-04-10 한민호)
        ...
        private void gvwMaster_CustomDrawCell(object sender, DevExpress.XtraGrid.Views.Base.RowCellCustomDrawEventArgs e)
        {
            ...
            if (status == "호출완료" || status == "입차" || status == "완료")
            {
                switch (gvwMaster.GetRowCellValue(e.RowHandle, "DIS_CLASS").ToString())
                {
                    ...
                    case "이송출하":
                        ...
                        break;
                    case "이송입고":
                        //신평경비실 요청에 의해 색깔을 약하게(2020-04-10 한민호)
            ...
            {
                //신평경비실 요청에 의해 이송입고는 호출완료라도 색깔 다르게 표시(2020-04-10 한민호)
                if (gvwMaster.GetRowCellValue(e.RowHandle, "DIS_CLASS").ToString() == "이송입고")
                    e.Appearance.BackColor = Color.Thistle;
                else
                    //신평경비실 요청에 의해 색깔 변경(2020-04-08 한민호)
